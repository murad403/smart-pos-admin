  ...
  return (
    <div className="space-y-6 pb-10 animate-fade-in">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">Shift History</h1>
          <p className="mt-1 text-sm text-slate-600">
            {user ? `View shift session logs for ${user.name || user.email}.` : "View shift session logs."}
          </p>
        </div>
      </div>
              ...
              >
                  ...
                  <div className="flex items-center gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-slate-400 font-medium">
                        Opened at: {new Date(session.createdAt).toLocaleTimeString([], { hour: "2-digit", minute: "2-digit" })}
                      </p>
                    </div>
                  </div>

                  <span
                    className={`text-[10px] font-extrabold tracking-wider uppercase px-2.5 py-1 rounded-full border w-fit ${
                ...
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 border-b border-slate-100 pb-4 mb-4">
                  <div className="bg-slate-50/50 border border-slate-100 p-3 rounded-xl">
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Opening Cash</p>
                    <p className="text-base font-extrabold text-slate-800 mt-1">
                      {formatCurrency(session.openingCashAmount)}
                    </p>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 p-3 rounded-xl">
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Closing Cash</p>
                    <p className="text-base font-extrabold text-slate-800 mt-1">
                      {formatCurrency(session.closingCashAmount)}
                    </p>
                  </div>
                  <div className="bg-slate-50/50 border border-slate-100 p-3 rounded-xl">
                    <p className="text-xs text-slate-400 font-bold uppercase tracking-wider">Discrepancy</p>
                    <p
                      className={`text-base font-extrabold mt-1 ${
                        session.closingDiscrepancy && parseFloat(session.closingDiscrepancy) !== 0
                ...
                <div className="space-y-3">
                  <p className="text-[11px] font-bold text-slate-450 uppercase tracking-wide">Checks Checklist</p>
                  <div className="flex flex-wrap gap-4">
                    ...
                    <div className="flex items-center gap-2">
                      <span className="text-xs font-semibold text-slate-500">Sales Confirmed:</span>
                ...
                {session.cashProofs && session.cashProofs.length > 0 && (
                  <div className="mt-5 pt-4 border-t border-slate-100">
                    <p className="text-[11px] font-bold text-slate-455 uppercase tracking-wide">Cash Proofs</p>
